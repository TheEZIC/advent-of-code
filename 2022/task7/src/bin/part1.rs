...
impl CardHand {
    ...
    fn get_type(&self) -> CardHandType {
        ...
        let types_count = values.len();

        return match *values.iter().max().unwrap() {
            5 => CardHandType::FiveOfKind,
            4 => CardHandType::FourOfKind,
            3 => if types_count == 2 { CardHandType::FullHouse } else { CardHandType::ThreeOfKind },
            2 => if types_count == 3 { CardHandType::TwoPair } else { CardHandType::OnePair },
